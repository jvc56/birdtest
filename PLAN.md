...
(2.2 s at the test volume), and counting distinct analysed racks alongside the
average best equity cost 3.3 s at a million racks on every detail view and every
live push, against 0.34 s for the average alone. Nothing
...
is what removes the piece most in need of unit tests. Task requests still carry
...
2. For each candidate in deficit order: reclaim its expired claims, then try to
   ...
   - **NeedsGenerationTransition** — leave generation only. The transition
...
batched `INSERT`s: 37 seconds against 54 on a developer machine), and a
...
server keys on `(game_number, pair_game_number, turn_number)` — which it must do
anyway.
...
the task's JSON request and go back in its JSON response. The `-writerackequitycsv`
