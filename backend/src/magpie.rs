...
mod tests {
    ...
    fn the_builders_json_magpie_prints_parses() {
        let printed = r#"{"magpie_version":"0.5.0","build_target":"nehalem","wmp_builder_version":1,"rit_builder_version":1,"klv_builder_version":1}"#;
        let builders: Builders = serde_json::from_str(printed).unwrap();
        assert_eq!(builders.magpie_version, "0.5.0");
