...
`MIN_MAGPIE_VERSION` defaults to `0.4.0`, which `birdtest-contribute` reports.
A checkout from before that reports `0.3.0` or lower, and every task is declined with
...
keep over plain HTTP. `min_magpie_version` defaults to `0.4.0`, the first MAGPIE
version whose results depend on nothing but the task; raise it whenever a
